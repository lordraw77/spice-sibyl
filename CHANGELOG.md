...
### Added
- **Nodi multimodali (roadmap fase 15.5)** — `audio.transcribe`, `image.ocr`, `image.generate` e `tts` in `app/workflow/nodes/multimodal.py`. I path sono workspace-relative e passano da `safe_workspace_path`, e `path` ha come default il node input come già faceva `doc.convert`, così un trigger `file.watch` può alimentarli senza ripetere `{{ $trigger.path }}`. Le due direzioni sono asimmetriche di proposito: i nodi che consumano un file restituiscono il testo, usabile direttamente da un'espressione; quelli che lo producono restituiscono il path scritto più i metadati del provider — restituire megabyte di base64 renderebbe illeggibile il log della run e finirebbe in ogni riga `node_run`. Nuovo `app/services/speech_service.py` per trascrizione e sintesi (servizio a sé che chiama litellm, sulla falsariga di `image_service`, invece di una coppia di metodi su `BaseProvider`: il parlato prende un file e restituisce un file, quindi infilarlo nei dieci provider di chat avrebbe voluto dire nove `NotImplementedError`). `image.ocr` non usa un motore OCR ma il provider layer con l'immagine come data URI, perché i modelli vision fanno meglio su screenshot e scrittura a mano. Nuove variabili: `SPEECH_TRANSCRIPTION_MODEL`, `SPEECH_TTS_MODEL`, `SPEECH_TTS_VOICE`, `VISION_OCR_MODEL`
- **Variante immagine `<versione>-browser` (roadmap fase 15.3)** — il nodo `browser` era shipped-but-broken da quando è stato scritto, perché Playwright non è nell'immagine backend. Continua a non esserci: Chromium e le sue librerie la portano da 1,03 GB a 2,3 GB e la maggioranza dei deployment non tocca mai quel nodo. `backend/Dockerfile.browser` li stratifica sull'immagine backend della stessa versione (`make docker/build-browser` / `docker/push-browser`, vedi `docs/deploy.md` § 1.1). **Non** è andato sul runner remoto della fase 14 come previsto dalla roadmap: `browser` non è fra i `_REMOTE_CAPABLE_TYPES` perché scrive gli screenshot nella workspace storage, quindi ha bisogno del contesto backend che un processo runner non riceve mai
