...
- **23.a — Shared conversation history across Telegram and web** — for linked users (`/link`), Telegram sessions are persisted as regular profile conversations instead of the current in-memory per-chat history: `/history` lists the profile's recent conversations (web ones included) with an inline keyboard to resume any of them, `/new` starts a fresh persisted conversation, and conversations started on Telegram appear live in the web sidebar (with a channel badge). Requires migrating the bot's session store onto `conversation_service` with per-chat "active conversation" tracking in `telegram_prefs`
- **23.b — MCP tools from Telegram** — expose the configured MCP servers (Phase 18) in the bot's tool loop: when tools are enabled for the chat, `mcp__<server>__<tool>` tools are merged into the Telegram completion requests exactly as in the web chat; `/tools` command lists available built-in/custom/MCP tools with per-chat ON/OFF toggle (persisted in `telegram_prefs`), and tool-call progress is surfaced in the live-edited streaming reply (⚙ tool name while executing)
