...
async def init_db() -> None:
    async with aiosqlite.connect(settings.db_path) as db:
        ...
        await db.commit()
        # Schema changes are versioned and recorded in schema_migrations, so a
        # boot only runs what this database is missing. See app/db/migrations.py
        # for how to add one.
        applied = await migrations.apply(db)
        if applied:
